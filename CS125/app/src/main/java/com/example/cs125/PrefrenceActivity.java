        ...
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.POST, builder.toString(), null,
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            Intent intent = new Intent(PrefrenceActivity.this, SearchActivity.class);
                            String success = response.getString("success");
//                            if everything correct it will jump to weighted page(Not emergency mode)
                            if (success.equals("true")){
                                String msg = response.getString("msg");
                                intent.putExtra("weighted",true);
                                Toast.makeText(PrefrenceActivity.this,msg, Toast.LENGTH_SHORT).show();
                                startActivity(intent);
                            }
//                            show the fail massage
                            else{
                                String msg = response.getString("msg");
